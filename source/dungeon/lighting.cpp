...
void DoLighting(unsigned lnum)
{
	...
	// Add light to (0;0)
	{
			...
			//if (radius_block <= MAX_LIGHT_DIST) {
				...
				temp_y = nYPos + 0;
				v = dark[radius_block];
// QMessageBox::critical(nullptr, "Error", QString("v%1, r%2 at%3:%4 o%5:%6 r%7").arg(v).arg(radius_block).arg(temp_x).arg(temp_y).arg(xoff).arg(yoff).arg(nRadius));
...
void TraceLightSource(int nXPos, int nYPos, int nRadius)
{
	...
	nRadius = 2 * (nRadius + 1) * 8 * 16 * 16;
	static_assert(INT_MAX / (2 * 8 * 16 * 16) > MAX_LIGHT_RAD, "Light tracing overflows in TraceLightSource.");
		...
		if (dx >= dy) {
			...
			do {
				...
				if (d >= dx) {
					...
					y1 += yinc;
					limit -= 1 * 1741; // 1 * 109; // 1 * 7;
				}
				x1 += xinc;
				limit -= 2 * 8 * 16 * 16;
				if (limit <= 0)
					break;
			} while (LightPos(x1, y1, (nRadius - limit) >> (1 + 4 + 4))); // * (MAX_OFFSET / (2 * 8)
		} else {
			// multiply by 2 so we round up
			dx *= 2;
			d = 0;
