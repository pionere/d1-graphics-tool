...
static void DRLG_L4Shadows()
{
	...
	for (x = 1; x < DMAXY; x++) {
		for (y = 1; y < DMAXY; y++) {
			...
			//} else if (bv == 78 || bv == 16 || bv == 5 || bv == 7) { -- not really necessary || bv == 83 - setpieces?
			//	if (dungeon[x - 1][y - 1] == 6) {
			//		dungeon[x - 1][y - 1] = 72;
			//	}
			}
...
static void L4AddHWall(int x, int y)
{
	...
	// 1, 4, 7, 8, 9, 10, 15, 18, 21, 22

	// 2, 5, 7, 8, 9, 11, 16, 19, 21, 23
}
...
static void DRLG_L4()
{
	...
	if (currLvl._dLevelIdx != DLV_HELL4) {
		DRLG_PlaceThemeRooms(7, 10, themeTiles, 8, true);
		DRLG_L4ThemeExitFix();
	}
