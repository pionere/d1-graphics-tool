	...
	for (j = DMAXY - 1; j > 0; j--) {
		for (i = DMAXX - 1; i > 0; i--) {
			...
			if (horizArch) {
				...
				if (!okB) {
					if (dungeon[i - 1][j - 1] == 13) {
						...
						if (dungeon[i - 1][j - 1] != 25 && dungeon[i - 1][j - 1] != 53)
			...
			if (vertArch) {
				...
				// case 140: replaceA = 127; okB = false; break;
...
static void DRLG_L1FixPreMap(int idx)
{
	...
	if (pSetPieces[idx]._sptype == SPT_BANNER) {
		...
		lm[2 + 37 * 25 + (19 / 2) + (30 / 2) * 37] = SwapLE16(3 << 8);
		lm[2 + 37 * 25 + (19 / 2) + (32 / 2) * 37] = SwapLE16(3 << 8);
