...
void DRLG_L2Shadows()
{
	...
	for (j = DMAXY - 1; j > 0; j--) {
		for (i = DMAXX - 1; i > 0; i--) {
			...
			bool vertArch = false;
			horizArch = (automaptype[dungeon[i][j]] & MAP_EAST_ARCH) != 0;
			vertArch = (automaptype[dungeon[i][j]] & MAP_WEST_ARCH) != 0;
			if (automaptype[dungeon[i][j]] & MAP_EAST_DOOR) {
			...
			if (horizArch) {
				...
				default:
                    dProgressWarn() << QString("Missing case %1 for horizontal arch %2 @%3:%4").arg(replaceB).arg(dungeon[i][j]).arg(DBORDERX + 2 * i).arg(DBORDERY + 2 * j);
					continue;
				}
			...
			if (pillar) {
				if (dungeon[i - 1][j] == 3) {
					...
					if (drlgFlags[i - 1][j] == 0 && (automaptype[dungeon[i - 1][j]] & MAP_TYPE) != MWT_NORTH_EAST && (automaptype[dungeon[i - 1][j]] & MAP_TYPE) != MWT_NORTH && dungeon[i - 1][j] != 9 && dungeon[i - 1][j] != 45 && dungeon[i - 1][j] != 50)
