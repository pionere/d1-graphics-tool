        ...
        for (int y = 0; y < this->height; y++) {
            ...
            for (int x = 0; x < this->width; x++) {
                ...
                if (!d1pix.isTransparent() && d1pix.getPaletteIndex() != 0) {
                    if (sx < x) {
                        dProgress() << QApplication::tr("gap %1 len %2").arg(sx, x - sx);
            ...
            if (sx != this->width) {
                dProgress() << QApplication::tr("gap %1 len %2").arg(sx, this->width - sx);
                gaps.push_back(QPair<int, int>(sx, this->width - sx));
            }

            for (auto it = gaps.begin(); it != gaps.end(); ) {
                if (it->second <= 2) {
                    dProgress() << QApplication::tr("short gap %1 len %2").arg(it->first, it->second);
            ...
            for (auto it = gaps.begin(); it != gaps.end(); it++) {
                dProgress() << QApplication::tr("long gap %1 len %2").arg(it->first, it->second);
