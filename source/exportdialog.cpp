...
bool ExportDialog::exportLevelTiles25D()
{
    ...
    ProgressDialog::start(PROGESS_DIALOG_STATE::ACTIVE, tr("Exporting %1 2.5d tiles...").arg(fileName), amount + 1);
    // single tile
...
bool ExportDialog::exportLevelTiles()
{
    ...
    ProgressDialog::start(PROGESS_DIALOG_STATE::ACTIVE, tr("Exporting %1 flat tiles...").arg(fileName), amount + 1);
    // single tile
    if (amount == 1 && tileFrom == 0) {
        // one file for the only tile (not indexed)
        QString outputFilePath = outputFilePathBase + this->getFileFormatExtension();
...
bool ExportDialog::exportLevelSubtiles()
{
    ...
    ProgressDialog::start(PROGESS_DIALOG_STATE::ACTIVE, tr("Exporting %1 subtiles...").arg(fileName), amount + 1);
    // single subtile
...
bool ExportDialog::exportFrames()
{
    ...
    ProgressDialog::start(PROGESS_DIALOG_STATE::ACTIVE, tr("Exporting %1 frames...").arg(fileName), amount + 1);
    // single frame
